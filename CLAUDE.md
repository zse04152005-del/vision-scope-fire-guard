...
│   ├── alarm_db.py             SQLite 告警数据库（持久化存储）
│   └── network_scanner.py      局域网摄像头设备扫描
├── ui/                         界面组件
...
| 2026-04-24 | [UX] 摄像头管理增强 | 重写 `camera_manager.py`：自动检测本地摄像头、RTSP 地址辅助填写、实时画面预览、视频文件导入、排序管理；非技术用户友好 |
| 2026-04-24 | [UX] 首次使用设置向导 | 新增 `ui/setup_wizard.py`：三步引导（欢迎→添加摄像头→完成），自动扫描+一键添加，替换原弹窗提示 |
| 2026-04-24 | [Deploy] 一键启动+打包 | 新增 `run.sh`/`run.bat` 一键启动脚本（自动检测依赖+安装）；`build_app.spec`+`build.sh` PyInstaller 打包配置 |
| 2026-04-25 | [F] SQLite 告警持久化 | 新增 `core/alarm_db.py`；告警写入 SQLite 数据库，重启自动加载今日历史记录；录像路径异步回填；关闭时安全断开 |
| 2026-04-25 | [F] 局域网设备扫描 | 新增 `core/network_scanner.py`；扫描子网 RTSP/HTTP 端口，发现 IP 摄像头；摄像头管理器新增「局域网扫描」Tab |
| 2026-04-25 | [Docs] README 全面重写 | 面向普通用户的 3 步上手指南；功能一览、摄像头接入说明、快捷键表 |
| 2026-04-25 | 新增 12 项单测 | alarm_db × 8、network_scanner × 4；115/115 绿 |
